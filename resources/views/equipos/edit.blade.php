...
<div class="flex items-center gap-3">
    ...
    <div>
        <h2 class="text-2xl font-bold text-gray-900 dark:text-white tracking-tight">Editar Equipo: {{ $equipo->nombre }}</h2>
        <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">Modifique la información del equipo y actualice su estado operativo.</p>
    </div>
</div>
@endsection
...
<div class="max-w-5xl mx-auto">
    ...
    <form action="{{ route('equipos.update', $equipo) }}" method="POST"
        ...
        <div class="px-6 py-5 border-b border-gray-100 dark:border-gray-700/50 bg-gray-50/50 dark:bg-gray-800/50">
            <h3 class="text-gray-900 dark:text-white font-bold text-lg">Configuración del Equipo</h3>
        </div>
        ...
        <div class="p-6 space-y-8">
            ...
            <div>
                <h4 class="text-sm font-extrabold uppercase tracking-wider text-gray-500 dark:text-gray-400 mb-4 border-b border-gray-100 dark:border-gray-700 pb-2">Información General</h4>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="space-y-2">
                        ...
                        <input type="text" name="codigo" value="{{ old('codigo', $equipo->codigo) }}" required
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="relative">
                            <select name="tipo_id" required
                                ...
                                @foreach($tipos as $t)
                                    <option value="{{ $t->id }}" {{ old('tipo_id', $equipo->tipo_id) == $t->id ? 'selected' : '' }}>
                                        {{ $t->nombre }}
                                    </option>
                                @endforeach
                            </select>
                    ...
                    <div class="space-y-2">
                        ...
                        <input type="text" name="marca" value="{{ old('marca', $equipo->marca) }}"
                            class="w-full px-4 py-3 bg-gray-50/50 dark:bg-gray-700/50 text-gray-800 dark:text-gray-200 border border-gray-200 dark:border-gray-655 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-505 transition-all duration-200 text-sm shadow-sm">
                    </div>
            ...
            <div>
                <h4 class="text-sm font-extrabold uppercase tracking-wider text-gray-500 dark:text-gray-400 mb-4 border-b border-gray-100 dark:border-gray-700 pb-2">Estado y Mantenimiento</h4>

                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">

                    {{-- Estado --}}
                    <div class="space-y-2">
                        <label class="block text-sm font-semibold text-gray-700 dark:text-gray-300">Estado Operativo</label>
                        <div class="relative">
                            <select name="estado" required
                                class="w-full appearance-none px-4 py-3 bg-gray-50/50 dark:bg-gray-700/50 text-gray-850 dark:text-gray-200 border border-gray-200 dark:border-gray-650 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 text-sm font-semibold shadow-sm transition-all duration-200">
                                <option value="operativo" {{ old('estado', $equipo->estado) == 'operativo' ? 'selected' : '' }}>
                                    Operativo
                                </option>
                                <option value="en_mantenimiento" {{ old('estado', $equipo->estado) == 'en_mantenimiento' ? 'selected' : '' }}>
                                    En Mantenimiento
                                </option>
                                <option value="con_falla" {{ old('estado', $equipo->estado) == 'con_falla' ? 'selected' : '' }}>
                                    Con Falla
                                </option>
                                <option value="dado_de_baja" {{ old('estado', $equipo->estado) == 'dado_de_baja' ? 'selected' : '' }}>
                                    Dado de Baja
                                </option>
                            </select>
                            <div class="absolute inset-y-0 right-0 flex items-center pr-3 pointer-events-none text-gray-400">
                    ...
                    {{-- Frecuencia --}}
                    <div class="space-y-2">
                        <label class="block text-sm font-semibold text-gray-700 dark:text-gray-300">Frecuencia de Mantenimiento (meses)</label>
                        <input type="number" name="frecuencia_mantenimiento"
                            value="{{ old('frecuencia_mantenimiento', $equipo->frecuencia_mantenimiento ?? 6) }}"
                            min="1" max="24"
                            class="w-full px-4 py-3 bg-gray-50/50 dark:bg-gray-700/50 text-gray-800 dark:text-gray-200 border border-gray-200 dark:border-gray-655 rounded-xl focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-505 transition-all duration-200 text-sm shadow-sm">
