...
@section('header')
    <div class="flex items-center gap-3">
        ...
        <div>
            ...
            <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">Actualice los datos específicos del registro.</p>
        </div>
    </div>
@endsection
            ...
            <form action="{{ route('mantenimientos.update', $mantenimiento) }}" method="POST" class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div class="relative">
                            <select name="equipo_id"
                                ...
                                @foreach($equipos as $e)
                                    <option value="{{ $e->id }}"
                                        {{ old('equipo_id', $mantenimiento->equipo_id) == $e->id ? 'selected' : '' }}>
                                        [{{ $e->codigo }}] - {{ $e->nombre }}
                                    </option>
                                @endforeach
                            </select>
                            ...
                            <div class="relative">
                                <select name="user_id"
                                    ...
                                    @foreach($usuarios as $u)
                                        <option value="{{ $u->id }}"
                                            {{ old('user_id', $mantenimiento->user_id) == $u->id ? 'selected' : '' }}>
                                            {{ $u->name }} (Técnico)
                                        </option>
                                    @endforeach
                                </select>
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="relative">
                            <select id="tipo" name="tipo"
                                class="w-full appearance-none border border-gray-200 dark:border-gray-650 rounded-xl px-4 py-3 bg-gray-50/50 dark:bg-gray-700/50 text-gray-800 dark:text-gray-200 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all duration-200">
                                <option value="correctivo" {{ old('tipo', $mantenimiento->tipo) == 'correctivo' ? 'selected' : '' }}>
                                    Correctivo
                                </option>
                                <option value="preventivo" {{ old('tipo', $mantenimiento->tipo) == 'preventivo' ? 'selected' : '' }}>
                                    Preventivo
                                </option>
                            </select>
                            <div class="absolute inset-y-0 right-0 flex items-center pr-3 pointer-events-none text-gray-400">
                    ...
                    <div class="space-y-2">
                        ...
                        <div class="w-full border border-gray-250 dark:border-gray-600 rounded-xl px-4 py-3 bg-gray-50/50 dark:bg-gray-750/30 flex items-center gap-2.5">
                            <span class="w-2.5 h-2.5 rounded-full {{ $mantenimiento->estado === 'en_proceso' ? 'bg-indigo-500 animate-pulse' : ($mantenimiento->estado === 'finalizado' ? 'bg-emerald-500' : 'bg-amber-500') }}"></span>
                            <span class="text-sm font-bold {{ $mantenimiento->estado === 'en_proceso' ? 'text-indigo-600 dark:text-indigo-400' : ($mantenimiento->estado === 'finalizado' ? 'text-emerald-600 dark:text-emerald-400' : 'text-amber-600 dark:text-amber-400') }}">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div class="space-y-2">
                        ...
                        <input type="date" name="fecha"
                            value="{{ old('fecha', optional($mantenimiento->fecha)->format('Y-m-d') ?? $mantenimiento->fecha) }}"
                            class="w-full border border-gray-200 dark:border-gray-650 rounded-xl px-4 py-3 bg-gray-50/50 dark:bg-gray-700/50 text-gray-800 dark:text-gray-200 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all duration-200">
                    </div>
                    ...
                    <div id="campoProxima" class="space-y-2">
                        ...
                        <input type="date" name="proxima_fecha"
                            value="{{ old('proxima_fecha', optional($mantenimiento->proxima_fecha)->format('Y-m-d') ?? $mantenimiento->proxima_fecha) }}"
                            class="w-full border border-gray-200 dark:border-gray-650 rounded-xl px-4 py-3 bg-gray-50/50 dark:bg-gray-700/50 text-gray-800 dark:text-gray-200 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all duration-200">
                    </div>
                ...
                <div class="flex items-center justify-end gap-3 pt-6 border-t border-gray-100 dark:border-gray-700">
                    <a href="{{ route('mantenimientos.index') }}"
                       class="px-5 py-3 border border-gray-200 dark:border-gray-600 text-gray-600 dark:text-gray-300 font-semibold rounded-xl hover:bg-gray-50 dark:hover:bg-gray-750 transition-all duration-200 text-sm">
                        Cancelar
                    </a>
                    <button type="submit"
...
tipo.addEventListener('change', toggleProxima);
toggleProxima();
