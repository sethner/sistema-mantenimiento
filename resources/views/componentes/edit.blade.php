...
<div class="max-w-xl mx-auto space-y-6">
    <div>
        <h2 class="text-2xl font-bold text-gray-800">Editar Componente</h2>
        <p class="text-sm text-gray-500">Actualiza el componente base y su categoría</p>
    </div>
    ...
    <div class="bg-white p-6 rounded-2xl shadow border">
        <form action="{{ route('componentes.update', $componente) }}" method="POST" enctype="multipart/form-data" class="space-y-5">
            @csrf
            @method('PUT')
            ...
            <div>
                ...
                <select name="tipo_id" required
                    ...
                    @foreach($tipos as $t)
                        <option value="{{ $t->id }}" {{ old('tipo_id', $componente->tipo_id) == $t->id ? 'selected' : '' }}>
                            {{ $t->nombre }}
                        </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <input type="text" name="nombre"
                    value="{{ old('nombre', $componente->nombre) }}"
                    required
                    class="w-full border px-3 py-2 rounded-lg focus:ring-2 focus:ring-blue-500">
            ...
            <div>
                ...
                <select name="categoria_id" required
                    ...
                    @foreach($categorias as $c)
                        <option value="{{ $c->id }}" {{ old('categoria_id', $componente->categoria_id) == $c->id ? 'selected' : '' }}>
                            {{ $c->nombre }}
                        </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                @if($componente->imagen)
                    <div class="mb-2">
                        <img src="{{ asset($componente->imagen) }}" alt="Imagen actual" class="w-16 h-16 object-cover rounded-md border">
                    </div>
                @endif
                <input type="file" name="imagen" accept="image/*"
                    class="w-full border px-3 py-2 rounded-lg focus:ring-2 focus:ring-blue-500">
                <p class="text-xs text-gray-500 mt-1">Sube una nueva imagen si deseas reemplazar la actual.</p>
